...
namespace LaEsperanza.Api.Services
{
    public class PushService : IPushService
    {
        ...
        public async Task SendToAllDevicesAsync(string title, string message)
        {
            try
            {
                await SendToAppleAsync(title, message).ConfigureAwait(true);
                await SendToAndroidAsync(title, message).ConfigureAwait(true);
            }
            catch (Exception ex)
            {
        ...
        public async Task SendToAndroidAsync(string title, string message)
        {
            try
            {
                foreach (Devices device in _dbContext.Devices.Where(d => d.DeviceValid.Value && d.DevicePushAuth != "Apple"))
                {
                    ...
                    string MessageToSend = JsonConvert.SerializeObject(new PushMessage { Title = title, Message = message });

                    try
                    {
                        PushClient.SendNotification(Subscription, MessageToSend, Details);
                    }
                    catch (Exception ex)
                    {
        ...
        public async Task SendToAppleAsync(string pushTitle, string pushMessage)
        {
            try
            {
                string data = await File.ReadAllTextAsync(_appleSettings.P8).ConfigureAwait(true);
                ...
                foreach (Devices d in _dbContext.Devices.Where(d => d.DeviceValid.Value && d.DevicePushAuth.Equals("Apple")))
                {
                    ...
                    using (StringContent stringContent = new StringContent(body, Encoding.UTF8, "application/json"))
                    {
                        ...
                        {
                            HttpResponseMessage resp = await client.SendAsync(request).ContinueWith(responseTask => { return responseTask.Result; }).ConfigureAwait(true);

                            if (resp != null)
                            {
                                string apnsResponseString = await resp.Content.ReadAsStringAsync().ConfigureAwait(true);
